...
namespace SimulationCore
{
    ...
    {
        private static readonly SimulationManager SimManager = new SimulationManager();
        private static List<SimulationHistory> _results = new List<SimulationHistory>();

        public static async Task Main(string[] args)
        {
            // Start simulations
            _results?.Clear();
            Parameters.SimulationParameters.ForEach(SimManager.AddSimulation);
            SimManager.Simulations.ForEach(s => s.Item2.ProgressChanged += SimulationUpdate);
            _results = await SimManager.StartSimulations();
            ...
            var values = _results.ToDictionary(
                ...
                result => JsonConvert.SerializeObject(
                    ...
                    {
                        Steps = Parameters.OnlyDownloadLastStep
                            ? new List<SimulationHistoryStep>() {result.Steps.LastOrDefault()}
                            : result.Steps
                    },
            ...
            var path = Path.Combine(downloadsPath, "results.zip");
            ...
            if (Parameters.SaveCsv)
            {
                ...
                bool writeHeader = !new FileInfo(resCSV).Exists;

                using (StreamWriter sw = File.AppendText(resCSV))
                {
                    if (writeHeader)
                    {
                        sw.WriteLine(
                            "Vehicle Type , Number of Vehicles , Number Of Orders, Delivery Time ,Delivery Cost");
                    }
                    ...
                    foreach (var r in _results)
                    {
                        ...
                        if (lastStep != null)
                        {
                            ...
                            {
                                sw.WriteLine(r.Parameters.VehicleTemplate.Name + "," + r.Parameters.NumberOfVehicles +
                                             "," +
    ...
    public static class Parameters
    {
        ...
        private const string Scenario2 =
        ...
        private const string Scenario3 =
        ...
        private const string Scenario4 =
        ...
        public static List<SimulationParameters> SimulationParameters = new List<SimulationParameters>(
            JsonConvert.DeserializeObject<List<Vehicle>>(Vehicles).Select(v => new SimulationParameters()
            {
                ...
                Graph = GraphImporterService.GenerateGraph(
                    JsonConvert.DeserializeObject<JsonGraphRootObject>(Scenario3)),
                VehicleTemplate = v,
